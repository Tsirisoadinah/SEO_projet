...
// Récupération des articles avec catégorie et journaliste
$stmt = $pdo->query(
  "SELECT a.*, c.libelle AS categorie, j.nom AS journaliste_nom
   ...
   ORDER BY a.creation DESC NULLS LAST, a.id_articles DESC"
);
      ...
      <div class="layout">
        <div class="main-column">
          <article class="lead-article">
            ...
            <h1 class="lead-title">
              <a href="<?= htmlspecialchars(slugify($lead['titre']), ENT_QUOTES, 'UTF-8') ?>.html">
            ...
            <div class="lead-links">
              <a href="<?= htmlspecialchars(slugify($lead['titre']), ENT_QUOTES, 'UTF-8') ?>.html">Voir l'article</a>
              <a href="edit_article-<?= (int)$lead['id_articles'] ?>.html">Modifier</a>
            </div>
          </article>
        </div>
          ...
          <div class="side-list">
            <?php foreach ($side as $a): ?>
              <article class="side-item">
                ...
                <h2 class="side-title">
                  <a href="<?= htmlspecialchars(slugify($a['titre']), ENT_QUOTES, 'UTF-8') ?>.html">
                ...
                <div class="article-actions">
                  <a href="<?= htmlspecialchars(slugify($a['titre']), ENT_QUOTES, 'UTF-8') ?>.html">Voir</a>
                  <a href="edit_article-<?= (int)$a['id_articles'] ?>.html">Modifier</a>
                </div>
              </article>
            <?php endforeach; ?>
          </div>
              ...
              <?php foreach ($bottom as $b): ?>
                <article class="bottom-item">
                  ...
                  <h3 class="bottom-title">
                    <a href="<?= htmlspecialchars(slugify($b['titre']), ENT_QUOTES, 'UTF-8') ?>.html">
                  ...
                  <div class="article-actions">
                    <a href="<?= htmlspecialchars(slugify($b['titre']), ENT_QUOTES, 'UTF-8') ?>.html">Voir</a>
                    <a href="edit_article-<?= (int)$b['id_articles'] ?>.html">Modifier</a>
                  </div>
                </article>
              <?php endforeach; ?>
            </div>
